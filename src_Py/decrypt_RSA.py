...
def get_secret():
    if len(sys.argv) <= 3:
        try:
            Sec = int(input(f"{sys.argv[0]}: Secret? "), 10)
        except Exception as err:
            print(f"{sys.argv[0]}: ERROR! Could not read secret.\n\t({err})\n", file=sys.stderr)
            raise Exception(TError.EBadName)
    else:
        Sec = int(sys.argv[3], 10)
...
def decrypt_file(aInfile, aOutfile):
    maxbound, _, privexponent = calculate_key_pair()
...
# Debug code
    ...
    for line in aInfile:  
        for currentchar in line:
            aOutfile.write(bytes(chr(encrypt_character(currentchar, privexponent, maxbound)),'latin-1'))

# Debug code
            if DEBUG_MODE:
                ecurrentchar = encrypt_character(currentchar, privexponent, maxbound)
...
def main():
    ...
    print(f"\n{sys.argv[0]}: File \"{input}\" decrypted (\"{output}\").\n", file=sys.stdout)
