...
class CEncryptRSA
	...
	public:
		CEncryptRSA(): LowLimit(RAND_MAX/2), HighLimit(RAND_MAX), PubStartPoint(RAND_MAX/2)
			{
			InicializeSeed();
		...
		CEncryptRSA(CALC_INT aLow, CALC_INT aHigh, CALC_INT aPubStart): LowLimit(aLow), HighLimit(aHigh), PubStartPoint(aPubStart)
			{
			InicializeSeed();
			GenerateRandomPrimes();
			Maxbound = CalculateKeyPair();
		...
		CEncryptRSA(CALC_INT aLow, CALC_INT aHigh, CALC_INT aPubStart, CALC_INT aBound): LowLimit(aLow), HighLimit(aHigh), PubStartPoint(aPubStart), Maxbound(aBound)
			{
			InicializeSeed();
