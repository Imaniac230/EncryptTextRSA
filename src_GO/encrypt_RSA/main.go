...
func isInLetterRange(character string) string {
	var res string
	if (character[0] >= 32) && (character[0] <= 126) {
		res = character
	} else {
		res = "Ee"
	}
...
func shareSecret(aVal calcint) calcint {
	res := aVal * StartPoint * calcint(2)
	return res
}
...
func calculateKeyPair(aPubE, aPrivE *calcint) calcint {
	...
	generateRandomPrimes(&Pprime, &Qprime)
...
func encryptFile(aInfile, aOutfile *os.File) calcint {
	var (
	...
	for err != io.EOF {
		...
		if err == nil {
			writechar[0] = byte(encryptCharacter(currentchar, pubexponent, maxbound))
			aOutfile.Write(writechar)

			/* Debug code */
			if DebugMode {
				currcharStr = string(currentchar)
				currecharStr = string(encryptCharacter(currentchar, pubexponent, maxbound))
				fmt.Fprintf(os.Stdout, "[%d(%s) > %d(%s)]%-12s", currentchar, isInLetterRange(currcharStr), encryptCharacter(currentchar, pubexponent, maxbound), isInLetterRange(currecharStr), "")
				count++
...
func main() {
	...
	fmt.Fprintf(os.Stdout, "\n%s: Secret! {%d}\n\n", os.Args[0], encryptFile(fin, fout))
